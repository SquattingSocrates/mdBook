...
(function codeSnippets() {
    ...
    const rustPlaygrounds = Array.from(
        document.querySelectorAll(".playground code.language-rust")
    );
        ...
        if (window.ace) {
            ...
            if (code_block.classList.contains("editable")) {
                ...
                editor.commands.addCommand({
                    ...
                    exec: (_editor) => run_code_from_block(playground_block),
                });
            }
        }
    }
    ...
    function run_js_code(code_block) {
        var result_block = code_block.querySelector(".result");
        if (!result_block) {
            result_block = document.createElement("code");
            result_block.className = "result hljs language-bash";
        ...
        fetch_with_timeout("http://localhost:4000/run_nodejs", {
                headers: {
                    "Content-Type": "application/json",
                },
                method: "POST",
            ...
            .then((response) => response.json())
            .then((response) => {
                if (!response.result.stderr && !response.result.stdout) {
                    result_block.innerText = "No output";
                    result_block.classList.add("result-no-output");
                } else {
                    result_block.innerText = `$ node script.js ${
            ...
            .catch(
                (error) =>
                (result_block.innerText =
                    "NodeJS Playground Communication: " + error.message)
            );
    }

    function run_code_from_block(code_block) {
        if (code_block.querySelector("code.language-rust")) {
            return run_rust_code(code_block);
        }
        return run_js_code(code_block);
    }
    ...
    Array.from(document.querySelectorAll(".playground")).forEach(function(
        ...
        if (window.ace && code_block.classList.contains("editable")) {
            ...
            var saveFileButton = document.createElement("button");
            saveFileButton.className = "fa fa-floppy-o";
            saveFileButton.title = "Save changes";
            saveFileButton.setAttribute("aria-label", saveFileButton.title);

            buttons.insertBefore(saveFileButton, buttons.firstChild);
